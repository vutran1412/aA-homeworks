...
def top_titles
  ...
  top_titles = Movie.find_by_sql([
    'SELECT id, title FROM movies WHERE score >= ?',
    9
  ])
end
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  star_wars = Movie.find_by_sql([
    'SELECT id, title, yr FROM movies WHERE title LIKE ?',
    '%Star Wars%'
  ])
...
def below_average_years
  ...
  below_average_year = Movie.find_by_sql([
    'SELECT yr, COUNT(*) AS bad_movies FROM movies WHERE score < ? GROUP BY yr ORDER BY COUNT(*)',
    5
  ])
